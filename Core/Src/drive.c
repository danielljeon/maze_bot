...
static uint16_t scale_pwm_duty(float drive_value) {
  ...
  // Scale [-1,1] -> [MIN, MAX].
  const float mag = fabsf(drive_value);
  const float scaled = mag * PWM_DUTY_DELTA + MIN_PWM_DUTY;
...
void drive(void) {
  ...
  if (fabsf(h_bridge_1_drive) < DRIVE_DEADBAND) {
  ...
  if (fabsf(h_bridge_2_drive) < DRIVE_DEADBAND) {
